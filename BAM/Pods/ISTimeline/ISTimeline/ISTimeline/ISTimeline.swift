...
open class ISTimeline: UIScrollView {
    ...
    fileprivate static let gap:CGFloat = 30.0
    
    open var pointDiameter:CGFloat = 30.0 {
        didSet {
            if (pointDiameter < 0.0) {
                pointDiameter = 0.0
            } else if (pointDiameter > 150.0) {
                pointDiameter = 150.0
            }
        }
    }
    ...
    open var bubbleRadius:CGFloat = 12.0 {
        didSet {
            if (bubbleRadius < 0.0) {
                bubbleRadius = 0.0
            } else if (bubbleRadius > 30.0) {
                bubbleRadius = 30.0
            }
        }
    }
    
    open var bubbleColor:UIColor = .darkGray//.init(red: 0.75, green: 0.75, blue: 0.75, alpha: 1.0)
